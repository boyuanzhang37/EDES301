#!/usr/bin/env python3
"""
PocketBeagle DDR-style game — 4x2 LED grid, analog joystick (VRy -> AIN0), push button on P2_10

Hardware:
- LEDs:
  Left column (rows top->bottom):  P2_02, P2_04, P2_06, P2_08
  Right column (rows top->bottom): P2_18, P2_20, P2_22, P2_24
- Joystick:
  VRy (analog) -> through voltage divider 4.7k/5.1k -> P1_19
  SW (press) -> P2_10
  Joystick VCC -> 3.3V, GND -> GND
- Buzzers:
  BUZZER_A (tempo) -> P2_15
  BUZZER_B (error) -> P2_16
- HT16K33 displays:
  Display A addr 0x70 on I2C bus 1 (P2_09 SCL, P2_11 SDA)
  Display B addr 0x71 on I2C bus 2 (P1_26 SCL, P1_28 SDA)
"""
...
# --------------------------
LED_PINS = [
    "P2_02","P2_04","P2_06","P2_08",   # left column
    "P2_18","P2_20","P2_22","P2_24"    # right column
]

JOY_SW_PIN = "P2_10"   # press button
VR_ADC_CHANNEL = 0      # AIN0 (P1_19 through voltage divider)
...
def configure_pins():
    ...
    # Buzzer pins (PWM-capable)
    os.system(f"config-pin {BUZZER_A_PIN} pwm")
    os.system(f"config-pin {BUZZER_B_PIN} pwm")
    # I2C pins (HT16K33 displays)
    os.system("config-pin P2_09 i2c")  # SCL
    os.system("config-pin P2_11 i2c")  # SDA
    os.system("config-pin P1_26 i2c")  # SCL
    os.system("config-pin P1_28 i2c")  # SDA
    # Analog input pin (VRy via voltage divider) — AIN pins usually do not need config-pin
# --------------------------
# Game constants
# --------------------------
SLOW_BPM = 90
...
# ADC thresholds (after voltage divider: 4.7k/5.1k)
VR_LEFT_THRESH = 1400
...
def setup_gpio():
    ...
    GPIO.setup(BUZZER_A_PIN, GPIO.OUT); GPIO.setup(BUZZER_B_PIN, GPIO.OUT)
    GPIO.output(BUZZER_A_PIN, GPIO.LOW); GPIO.output(BUZZER_B_PIN, GPIO.LOW)
...
def play_tone(pin, freq, duration):
    ...
    t=threading.Thread(target=_t); t.daemon=True; t.start()
...
def read_adc_raw(channel=0):
    ...
    try:
        ...
        raise RuntimeError(f"Failed to read ADC channel {channel} (path {path}): {e}")

def joystick_from_analog():
    raw = read_adc_raw(VR_ADC_CHANNEL)
    left = raw < VR_LEFT_THRESH
    right = raw > VR_RIGHT_THRESH
    press = not GPIO.input(JOY_SW_PIN)  # pressed = LOW
...
class DDRGame:
    def __init__(self):
        ...
        self.running = False
        self.score = 0
        self.active = deque()   # (col,row)
        self.lock = threading.Lock()
        self.stop_event = threading.Event()
        self.show_bpm()

    def show_bpm(self): self.display_a.show_number(self.bpm, leading_zero=True); self.display_b.show_blank()
